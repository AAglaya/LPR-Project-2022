...
while not finished:                                                                                                     # Пока игра не закончилась
    if name.get_act_mode():                                                                                             # Если активирован режим взаимодействия
        ...
        if finish_game:                                                                                                 # Если игра заканчивается
            ...
            finished = True
            name.set_act_mode(False)
        screen.fill((0, 0, 0))                                                                                          # Рисовать интерфейс взаимодействия
    ...
    if name.get_fight_mode():                                                                                           # Если персонаж в режиме боя
        if name.get_location() == 2:                                                                                    # Если персонаж дерётся в локации 2
            ...
            if npc2.get_number_of_shots_type1() <= 10:                                                                  # Если npc2 ещё не сделал 10 залпов
                if not start_fire_1 or (start_fire_1 and timer - time_fire >= 1):                                       # Если огонь ещё не открыт или между залпами прошла секунда
                ...
                if heart.get_hp() > 0:                                                                                  # Если hp > 0
                    mark = font_style.render("Nikolaenko: Your mark is " + str(heart.get_hp() * 2) + '/10', True,       # Сгенерировать сообщение об оценке
                                             (255, 255, 255))
    ...
    for i in pygame.event.get():
        ...
        if pygame.key.get_pressed()[pygame.K_w] and not name.get_act_mode():                                            # Движение персонажа, если он не в режиме взаимодействия
            name.move_up(map)
        ...
        if name.get_location() == 2 and name.small_dist_to_npc(npc2) and npc2.is_alive():                               # Аналогично для локации 2
            ...
            if i.type == pygame.MOUSEBUTTONDOWN:
                ...
                elif 240 <= i.pos[0] <= 480 and 680 <= i.pos[1] <= 760 and not name.get_fight_mode():                   # Поговорить можно только когда не активирован режим боя
